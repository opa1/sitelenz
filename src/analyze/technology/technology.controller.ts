...
// below, matching the pattern the old AnalysesController used.
@SkipThrottle({ 'analysis-create': true })
@ApiTags('analyze')
@Controller('v1/analyze/technology')
export class TechnologyController {
  ...
  @ApiOperation({
    summary: "Analyze a website's technology stack",
    description:
      'Queues a lightweight technology-detection job (plain HTTP fetch, no browser). Detects frontend frameworks, CMS, CDN, analytics, payment providers, CSS libraries, and fonts. Requires an x402 payment of $0.01, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the technology analysis result',
    description:
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed technology analyze job',
    description:
