...
@ApiTags('analyze')
@Controller('v1/analyze/ux-accessibility')
export class UxAccessibilityController {
  ...
  @ApiOperation({
    summary: "Analyze a website's UX and accessibility",
    description:
      'Queues a full Playwright + Lighthouse UX/accessibility-analysis job. Evaluates mobile viewport configuration, navigation structure, form detection, CTA presence, reading metrics, and Lighthouse accessibility audit scores. Requires an x402 payment of $0.01, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        ...
        resource: {
          url: '/v1/analyze/ux-accessibility',
          description: 'SiteLenz ux-accessibility analysis',
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get ux-accessibility analyze job status and progress' })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the ux-accessibility analysis result',
    description:
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed ux-accessibility analyze job',
    description:
