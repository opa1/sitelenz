...
import { ANALYZE_STANDARD_PRICE_USD } from '../../x402/x402.constants';
...
const ENDPOINT = 'standard' as const;

@SkipThrottle({ 'analysis-create': true })
@ApiTags('analyze')
@Controller('v1/analyze/standard')
export class StandardController {
  ...
  @SetAnalyzePrice(ANALYZE_STANDARD_PRICE_USD)
  @UseGuards(X402Guard)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Run a full standard website analysis',
    description:
      'Queues the full pipeline: a Playwright crawl, a desktop screenshot, all six analyzers (technology, SEO, security, performance, business, UX), and an AI interpretation. Requires an x402 payment of $0.40, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    status: 402,
    description: 'Payment required - $0.40, raw x402 PaymentRequired body',
    schema: {
      example: {
        x402Version: 2,
        error: 'Payment required',
        resource: {
          url: '/v1/analyze/standard',
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get standard analyze job status and progress' })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the standard analysis result',
    description:
      'Returns the stored result once the job has completed: website info, all six analyzer sections, ai, and a desktop screenshot. No x402 payment is required here - the job was already paid for when it was created.',
  })
  @ApiParam({ name: 'id', example: 'sl_aj_01j8z9k3n8v5w6x7y8z9a0b1c2' })
  @ApiResponse({
    status: 200,
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed standard analyze job',
    description:
