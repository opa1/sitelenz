...
import { ANALYZE_PERFORMANCE_PRICE_USD } from '../../x402/x402.constants';
...
@ApiTags('analyze')
@Controller('v1/analyze/performance')
export class PerformanceController {
  ...
  @SetAnalyzePrice(ANALYZE_PERFORMANCE_PRICE_USD)
  @UseGuards(X402Guard)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Analyze a website's performance",
    description:
      'Queues a full Playwright + Lighthouse performance-analysis job. Runs a full Lighthouse audit to collect Core Web Vitals (LCP, CLS, FCP, TBT, TTFB), page weight, resource inventory, and third-party domain analysis. Requires an x402 payment of $0.02, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    status: 402,
    description: 'Payment required - $0.02, raw x402 PaymentRequired body',
    schema: {
      example: {
        x402Version: 2,
        error: 'Payment required',
        resource: {
          url: '/v1/analyze/performance',
          description: 'SiteLenz performance analysis',
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get performance analyze job status and progress' })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the performance analysis result',
    description:
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed performance analyze job',
    description:
