...
import { ANALYZE_LIGHTWEIGHT_PRICE_USD } from '../../x402/x402.constants';
...
const ENDPOINT = 'business' as const;

@SkipThrottle({ 'analysis-create': true })
@ApiTags('analyze')
@Controller('v1/analyze/business')
export class BusinessController {
  ...
  @SetAnalyzePrice(ANALYZE_LIGHTWEIGHT_PRICE_USD)
  @UseGuards(X402Guard)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Analyze a website's business signals",
    description:
      'Queues a lightweight business-analysis job (plain HTTP fetch, no browser). Extracts business name, description, contact info, social links, pricing signals, CTA text, and business model indicators. Requires an x402 payment of $0.01, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        ...
        resource: {
          url: '/v1/analyze/business',
          description: 'SiteLenz business analysis',
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get business analyze job status and progress' })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the business analysis result',
    description:
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed business analyze job',
    description:
