...
import { ANALYZE_HEAVY_PRICE_USD } from '../../x402/x402.constants';
...
@ApiTags('analyze')
@Controller('v1/analyze/screenshots')
export class ScreenshotsController {
  ...
  @SetAnalyzePrice(ANALYZE_HEAVY_PRICE_USD)
  @UseGuards(X402Guard)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Capture desktop and mobile screenshots of a website',
    description:
      'Queues a Playwright screenshot job. Captures full desktop (1280x720) and mobile (390x844) viewport screenshots via headless Chromium and returns Cloudinary-hosted image URLs. Requires an x402 payment of $0.01, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        ...
        resource: {
          url: '/v1/analyze/screenshots',
          description: 'SiteLenz screenshots analysis',
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the screenshots result',
    description:
      'Returns { desktop, mobile } once the job has completed, each either a Cloudinary-hosted screenshot or null if that capture failed. No x402 payment is required here - the job was already paid for when it was created.',
  })
  @ApiParam({ name: 'id', example: 'sl_aj_01j8z9k3n8v5w6x7y8z9a0b1c2' })
  @ApiResponse({
    status: 200,
    description: 'Job completed - { desktop, mobile } result JSON',
    schema: {
      type: 'object',
      example: {
        desktop: {
        ...
        mobile: {
          url: 'https://res.cloudinary.com/sitelenz/image/upload/v1/sitelenz/screenshots/sl_aj_xxx/sl_aj_xxx-mobile.png',
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed screenshots analyze job',
    description:
