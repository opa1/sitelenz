...
const ENDPOINT = 'security' as const;

@SkipThrottle({ 'analysis-create': true })
@ApiTags('analyze')
@Controller('v1/analyze/security')
export class SecurityController {
  ...
  @ApiOperation({
    summary: "Analyze a website's HTTP security posture",
    description:
      'Queues a lightweight security-analysis job (plain HTTP fetch, no browser). Audits HTTP security headers (HSTS, CSP, X-Frame-Options, Referrer-Policy), HTTPS status, and mixed content. Requires an x402 payment of $0.01, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        ...
        resource: {
          url: '/v1/analyze/security',
          description: 'SiteLenz security analysis',
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get security analyze job status and progress' })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the security analysis result',
    description:
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed security analyze job',
    description:
