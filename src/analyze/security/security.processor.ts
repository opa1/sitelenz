...
import { ANALYZE_SECURITY_QUEUE } from '../../queue/queue.constants';
...
@Processor(ANALYZE_SECURITY_QUEUE, {
  concurrency: 5,
  lockDuration: 120_000,
  lockRenewTime: 30_000,
})
export class SecurityProcessor extends BaseAnalyzeProcessor {
  ...
  async process(job: Job<AnalyzeJobData>): Promise<void> {
    ...
    try {
      ...
      const result = await this.securityAnalyzer.analyze(rawObservations, {
        deep: false,
      });
