...
import { ANALYZE_FULL_PRICE_USD } from '../../x402/x402.constants';
...
const ENDPOINT = 'full' as const;

@SkipThrottle({ 'analysis-create': true })
@ApiTags('analyze')
@Controller('v1/analyze/full')
export class FullController {
  ...
  @SetAnalyzePrice(ANALYZE_FULL_PRICE_USD)
  @UseGuards(X402Guard)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Run a comprehensive deep website analysis',
    description:
      'Queues the full pipeline in deep mode: a Playwright crawl, desktop and mobile screenshots, all six analyzers in deep mode (JS dependency tree, robots.txt/sitemap/hreflang, TLS/cookie audit, resource inventory/third-party analysis, nav/footer/pricing extraction, mobile UX/reading experience), and an expanded AI interpretation. Requires an x402 payment of $0.80, enforced via the PAYMENT-SIGNATURE header.',
  })
  ...
  @ApiResponse({
    status: 402,
    description: 'Payment required - $0.80, raw x402 PaymentRequired body',
    schema: {
      example: {
        x402Version: 2,
        ...
        resource: {
          url: '/v1/analyze/full',
          description: 'SiteLenz full analysis',
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get full analyze job status and progress' })
  ...
  @Get(':id/result')
  @ApiOperation({
    summary: 'Get the full analysis result',
    description:
      'Returns the stored result once the job has completed: website info, all six analyzer sections in deep mode, ai, and desktop + mobile screenshots. No x402 payment is required here - the job was already paid for when it was created.',
  })
  @ApiParam({ name: 'id', example: 'sl_aj_01j8z9k3n8v5w6x7y8z9a0b1c2' })
  @ApiResponse({
    status: 200,
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Retry a failed full analyze job',
    description:
